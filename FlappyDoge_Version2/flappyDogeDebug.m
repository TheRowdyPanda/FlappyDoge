...
#import "flappyDogeDebug.h"


@interface flappyDogeDebug () <SKPhysicsContactDelegate>
...
@implementation flappyDogeDebug
NSArray * _dogeFrames;
BOOL inJump;
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        self.background.size = CGSizeMake(340, 480);
        self.background.position = CGPointMake(170, 240);
...
-(void)dogeJump{
    ...
    SKAction * rotate = [SKAction rotateToAngle:(M_PI_2)*(90./360.) duration:(duration*.4)];
    SKAction *groupAction = [SKAction group:@[actionMoveUp, rotate]];
    [_doge runAction:[SKAction sequence:@[groupAction, actionMoveDown]]];

    
    [NSTimer scheduledTimerWithTimeInterval:duration*2.2
                                     target:self
                                   selector:@selector(jumpFinished)
                                   userInfo:nil
                                    repeats:NO];
